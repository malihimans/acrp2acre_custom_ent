...
def process_cluster(cluster, mc):
    ...
    sku_name = cluster.sku.name
    if sku_name == 'Basic':
        replicas_per_master = 0
    elif sku_name == 'Standard':
        replicas_per_master = 1
    elif sku_name == 'Premium':
        replicas_per_master = getattr(cluster, 'replicas_per_master', 0) or 1
    else:
        # Enterprise, EnterpriseFlash, and AMR all use 1 replica when HA is enabled
        replicas_per_master = 1

    cluster_rows = []

    if cluster.type == 'Microsoft.Cache/redisEnterprise' and sku_name not in amrClusterInfo['SKU']:
        # Enterprise E-series and EnterpriseFlash tiers — cluster-level metrics, no shard suffix
        cluster_info = lookup_enterprise_sku_capcity(sku_name, cluster.sku.capacity)
        if not isinstance(cluster_info, dict):
            # SKU not in acreClusterInfo lookup table (e.g. EnterpriseFlash); skip shard details
            master_shards = 'N/A'
        else:
            master_shards = cluster_info['MasterShards']

        non_metrics = [
            get_resource_group(cluster),
            cluster.location,
            cluster.name,
            f"{sku_name.split('_')[-1]}-Capacity{cluster.sku.capacity}",
            f"{sku_name.rsplit('_', 1)[0]}",
            replicas_per_master,
            master_shards
        ]

        shard_ops_metrics = round(get_max_metrics(mc, cluster.id, "operationspersecond"), 0)
        shard_memory_metrics = round(get_max_metrics(mc, cluster.id, "usedmemory") / 1024 / 1024, 2)  # bytes to MB
        shard_connection_metrics = get_max_metrics(mc, cluster.id, "connectedclients")
        cluster_rows.append(non_metrics + [0, shard_ops_metrics, shard_memory_metrics, shard_connection_metrics])

    elif cluster.type == 'Microsoft.Cache/redisEnterprise' and sku_name in amrClusterInfo['SKU']:
        # Azure Managed Redis (AMR) — cluster-level metrics, no shard suffix
        non_metrics = [
            get_resource_group(cluster),
            cluster.location,
            cluster.name,
            f"{sku_name.split('_')[-1]}",           # e.g. "B1", "M10", "X5"
            f"{sku_name.split('_')[0]}",             # e.g. "Balanced", "MemoryOptimized"
            replicas_per_master,
            1
        ]
    ...
    elif cluster.type == 'Microsoft.Cache/Redis':
        ...
        for shard_id in range(cluster_shard_count):
            ...
            shard_memory_metrics = round(get_max_metrics(mc, cluster.id, f"usedmemory{shard_id}") / 1024 / 1024, 2)  # bytes to MB
