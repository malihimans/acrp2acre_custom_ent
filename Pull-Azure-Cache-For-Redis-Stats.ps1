...
$OUTPUT_FILE_NAME = Join-Path $OutDir "AzureStats.csv"
...
# AMR SKU names — used to distinguish AMR from Enterprise E-series within the same resource type
$amrSkus = @(
    "GeneralPurpose_G3", "GeneralPurpose_G5",
    ...
    "ComputeOptimized_X50", "ComputeOptimized_X100", "ComputeOptimized_X150", "ComputeOptimized_X250",
    "ComputeOptimized_X350", "ComputeOptimized_X500", "ComputeOptimized_X700",
...
foreach ($subscription in $subscriptions) {
    ...
    # Get all Azure Cache for Redis Enterprise instances (includes Enterprise E-series and AMR)
    ...
    # Add SubscriptionID property to Enterprise/AMR instances and add to main array
...
foreach ($instance in $redisInstances) {
    ...
    if ($instance.ResourceType -eq "Microsoft.Cache/redisEnterprise" -and $instance.Sku.Name -notin $amrSkus) {
        # --- Enterprise E-series (and EnterpriseFlash) ---
        $skuInfo = Lookup-Enterprise-SKU-Capacity $instance.Sku.Name $instance.Sku.Capacity
        $masterShards = if ($null -ne $skuInfo) { $skuInfo.MasterShards } else { "N/A" }
        ...
        $clusterRow = [ordered]@{
            ...
            "SKU Capacity"       = "$($instance.Sku.Name.Split('_')[-1])-Capacity$($instance.Sku.Capacity)"
            "SKU Name"           = $instance.Sku.Name.Split('_')[0]   # e.g. "Enterprise", "EnterpriseFlash"
            "Replicas Per Master" = 1
            "Shard Count"        = $masterShards
            "Shard Number"       = 0
    ...
    } elseif ($instance.ResourceType -eq "Microsoft.Cache/redisEnterprise" -and $instance.Sku.Name -in $amrSkus) {
        ...
        $clusterRow = [ordered]@{
            ...
            "Region"             = $instance.Location
            "DB Name"            = $instance.Name
            "SKU Capacity"       = $instance.Sku.Name.Split('_')[-1]  # e.g. "B1", "M10", "X5"
            "SKU Name"           = $instance.Sku.Name.Split('_')[0]   # e.g. "Balanced", "MemoryOptimized"
            "Replicas Per Master" = 1
            "Shard Count"        = 1
            "Shard Number"       = 0
            "Avg Ops/Sec"        = $metrics.AvgOpsPerSec
        ...
        # --- OSS Redis (Basic / Standard / Premium) ---
        # API returns null if shard count is 1, so default to 1
        $shardCount = if ($null -eq $instance.ShardCount) { 1 } else { $instance.ShardCount }
        ...
        0..($shardCount - 1) | ForEach-Object {
            ...
            $usedMemory = ((Get-Max-Metric $instance.Id "usedmemory$($_)") / 1024 / 1024).ToString("F2")  # bytes to MB
            ...
            $clusterRows.Add([PSCustomObject]$clusterRow) | Out-Null  # PSCustomObject cast required for Windows PowerShell 5.1
        }
    }
}
